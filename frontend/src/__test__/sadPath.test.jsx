...
describe('sadPath', () => {
    ...
    it('should generate an error upon login click', async () => {
        ...
        mock.onPost(`http://z5363575-presto-deploy-frontend-rho.vercel.app:${BACKEND_PORT}/admin/auth/login`).reply(500);
        await waitFor(() => expect(screen.getByTestId('error-message')).toHaveTextContent('Error: Invalid username or password'));
        mock.reset();
    })
    // Register problems
    // create an account and then try again to login with the wrong password
    // create an account and then try again to login with the right password
})
